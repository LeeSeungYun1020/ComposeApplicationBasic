...
fun HomeScreen(
    ...
    Scaffold(scaffoldState = scaffoldState, topBar = {
        ...
        InsetAwareTopAppBar(title = { Text(text = title) }, navigationIcon = {
...
private fun PostList(
    posts: List<Post>, navigateToArticle: (postId: String) -> Unit, modifier: Modifier = Modifier
) {
    val postsHistory = posts.subList(0, 3)
    val postsPopular = posts.subList(3, 5)

    LazyColumn(
        modifier = modifier, contentPadding = rememberInsetsPaddingValues(
...
private fun PostListPopularSection(
    ...
    Column {
        ...
        LazyRow(contentPadding = PaddingValues(end = 16.dp)) {
            items(posts) { post ->
                PostCardPopular(
                    post, navigateToArticle, Modifier.padding(start = 16.dp, bottom = 16.dp)
                )
            }
        }
        PostListDivider()
...
fun PreviewHomeScreen() {
    JetnewsTheme {
        HomeScreen(posts = PostsRepository().getPosts(),
